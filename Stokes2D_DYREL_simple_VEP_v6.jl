...
function solve_stress(c, ηvep, εII, τII, τ0, P, P0, λ, dt)
    ...
    τII    = x[1]
    λ      = x[3]
    ηeff   = iszero(τII) ? zero(τII) : (τII - λ * ηvep) / (2 * εII)
    return τII, ηeff, λ
end

function solve_stress!(TII, ηvep, λ, composites, Phase, EII, TII0, Pt, Pt0, Δt)
    for i in eachindex(ηvep)
        TII[i], ηvep[i], λ[i] = solve_stress(composites[Phase[i]], ηvep[i], EII[i], TII[i], TII0[i], Pt[i], Pt0[i], λ[i], Δt)
    end
    nothing
end
...
@views function Stokes2D_VEP(n)
    ...
    c1 = let
        viscous     = LinearViscosity(1e0)
        viscous_reg = LinearViscosity(1e-2)
        elastic     = IncompressibleElasticity(G0)
        plastic     = DruckerPrager(C, ϕ, 0e0)
    ...
    c2 = let
        viscous     = LinearViscosity(1e0)
        viscous_reg = LinearViscosity(1e-2)
        elastic     = IncompressibleElasticity(Gi)
        plastic     = DruckerPrager(C, ϕ, 0e0)
    ...
    # Effective viscosity
    Phase_c = ones(Int, ncx, ncy)
    Phase_v = ones(Int, ncx+1, ncy+1)
    Phase_c[(xc).^2 .+ (yc').^2 .< radi^2 ] .= 2
    Phase_v[(xv).^2 .+ (yv').^2 .< radi^2 ] .= 2
    ...
    for it=1:nt
        ...
        @time for itPH = 1:1000
            ...
            TIIc0 .= sqrt.(0.5.*(Txx0.^2  .+ Tyy0.^2  .+ (.-Txx0.-Tyy0).^2)   .+ Txy0c.^2 )
            TIIv0 .= sqrt.(0.5.*(Txxv0.^2 .+ Tyyv0.^2 .+ (.-Txxv0.-Tyyv0).^2) .+ Txy0.^2 )
            # Plasticity
            # λ̇c            .= 0.
            # λ̇v            .= 0.
            ...
            solve_stress!(TIIc, ηvep_c, λ̇c, composites, Phase_c, EIIc, TIIc0, Pt, Pt0, Δt)
            solve_stress!(TIIv, ηvep_v, λ̇v, composites, Phase_v, EIIv, TIIv0, Ptv, Pt0v, Δt)
            Txx   .= 2.0.*ηvep_c.*(Exx .+ Txx0./(2*Gc*Δt))
            Tyy   .= 2.0.*ηvep_c.*(Eyy .+ Tyy0./(2*Gc*Δt))
            Txy   .= 2.0.*ηvep_v.*(Exy .+ Txy0./(2*Gv*Δt))
            Txxv  .= 2.0.*ηvep_v.*(Exxv .+ Txxv0./(2*Gv*Δt))
