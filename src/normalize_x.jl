...
""" 
    xnorm = normalisation_x(c, char_τ=1.0, char_ε=1.0)

Initial guess for the local solution vector `x` with given characteristic stress and strainrates values
"""
function normalisation_x(c,  char_τ=1.0, char_ε=1.0)
    eqs = generate_equations(c)
    x0  = normalisation_x(x_keys(eqs),  char_τ, char_ε)
    return SVector{length(x0)}(x0)
end

@generated function normalisation_x(x_keys::NTuple{N,Symbol}, char_τ, char_ε)  where {N}
    return quote
        @inline
        f = Base.@ntuple $N i -> _normalize_x_value(Val(x_keys[i]), char_τ, char_ε )
    end
end
