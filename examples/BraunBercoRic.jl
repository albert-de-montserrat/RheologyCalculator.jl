...
function BraunBercoRic()
    ...
    P = 5.0e9
    Ebg = 1.0e-15   # strain rate
    Vbg = 2 * H * Ebg # velocity
    dbg = 1.0e-3    # grain size
    G = 6.0e10    # Shear modulus
    k = 3
    rho = 3300
    c = 1000
    R = 8.314
    ...
    Qdis = 530.0e3 # activation energy (gas constant included)
    Vadis = 14.0e-6
    ...
    f_I = 1.0e-4 # fraction of deformational work going into grain size reduction
    ...
    anim = @animate for it in 1:nt
        ...
        @. GI = (q / p * exp(-Hg_eff ./ R ./ Tv) * (Ag / 250)) * (1.0e-6)^q # constant
        ...
        for iter in 1:500000
            ...
            dtaut = rhoi * dy^2 / Kappa ./ 2.1 .* cfl / 70.0e6
            # #         L=H
            # #         kT=k
            # #
            # #         rhoc_i = 1
        ...
        if mod(it, 10) == 0 || it == 1
            ...
            p1 = Plots.plot(title = "Vx @ $(@sprintf("%1.6f", time / Ma)) Ma", xlabel = "Vx [cm/y]", ylabel = "y [m]")
            p1 = Plots.plot!(Vx * cma, yc, label = "")
            p2 = Plots.plot(title = "Eii @ $(@sprintf("%1.6f", time / Ma)) Ma", xlabel = "log10 Exy [1/s]", ylabel = "y [m]", xlims = (-20, -10))
            p2 = Plots.plot!(log10.(Eii), yv, label = "")
            p3 = Plots.plot(title = "eta @ $(@sprintf("%1.6f", time / Ma)) Ma", xlabel = "log10 eta [Pa.s]", ylabel = "y [m]", xlims = (15, 25))
            p3 = Plots.plot!(log10.(eta_diss), yv, label = "")
            p4 = Plots.plot(title = "d @ $(@sprintf("%1.6f", time / Ma)) Ma", xlabel = "log10 d [m]", ylabel = "y [m]", xlims = (-6, -2))
            p4 = Plots.plot!(log10.(d), yv, label = "")
            display(Plots.plot(p1, p2, p3, p4))
        end
    end
    return gif(anim, "GSE_Localization1D.gif", fps = 40)
end
