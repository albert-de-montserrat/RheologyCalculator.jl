...
# physics
R         = 8.314   
P         = 1e7       # [Pa]             <--- should be the input from the code
d_o       = 1e-3      # d: grain size[m] <--- should be the input from the code
G         = 3.5e9     # Elastic modulus (E/1+nu/2 = 1e9/(1+0.3)/2)
Coh       = 1e7       # Von Mises yield
...
dt        = 1e7       # physical time step value
litmax    = 100
tol       = 1e-15
nT        = 100
nE        = 101
...
A1_disl    = 4e4*10.0^(-6*n_disl) # A_disl: preexponential constant[1/s*Pa^4]  ######### ici l'unite c'est Pa^(-n_disl).s^(-1)
...
for it=1:1  #nt
    ...
    while lit<litmax
        ...
        if (lit==1) rese0 = rese; resd0 = resd end
        ...
        if series == 0
            ...
            dr_d_deta   = @. 2*d.^2. *gse.*reduct.*(Eii_disl.*lam_disl.*(Exx.^2. *n_disl + Exx.^2 + 2*Exy.^2. *n_disl + 2*Exy.^2 + Eyy.^2. *n_disl + Eyy.^2 + Ezz.^2. *n_disl + Ezz.^2) + Eii_gbs.*lam_gbs.*(Exx.^2. *n_gbs + Exx.^2 + 2*Exy.^2. *n_gbs + 2*Exy.^2 + Eyy.^2. *n_gbs + Eyy.^2 + Ezz.^2. *n_gbs + Ezz.^2))./(Eii.*c.*gam)
            dr_d_dd     = @. (Eii.*c.*d.*gam + dt.*gse.*(Eii.*c.*dgg.*gam.*growth.*(p - 1) + 2*d.^2. *eta_ve.*reduct.*(2*Eii_disl.*lam_disl - Eii_gbs.*lam_gbs.*m_gbs + 2*Eii_gbs.*lam_gbs).*(Exx.^2 + 2*Exy.^2 + Eyy.^2 + Ezz.^2)))./(Eii.*c.*d.*dt.*gam)
        elseif series == 1
    ...
    if series==1
        ...
        eps_tmp = (1. ./Eii_basal .+ 1. ./Eii_gbs).^(-1)
        if diff==1      @. mech[Eii_diff  > eps_tmp   && Eii_diff  > Eii_disl && Eii_diff > Eii_el  && Eii_diff > Eii_el] = 1 end
        if disl==1      @. mech[Eii_disl  > Eii_diff  && Eii_disl  > eps_tmp  && Eii_disl > Eii_el  && Eii_disl > Eii_el] = 4 end
        if bas_gbs==1   @. mech[eps_tmp   > Eii_diff  && eps_tmp   > Eii_disl && eps_tmp  > Eii_el  && eps_tmp  > Eii_el] = 2 end
        if von_mises==1 @. mech[Eii_vm    > Eii_diff  && Eii_vm    > Eii_disl && Eii_vm   > eps_tmp && Eii_vm   > Eii_el] = 5 end
