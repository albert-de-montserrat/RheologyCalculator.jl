...
compute_Q(τ, P, ψ, σt) = sqrt(τ^2 + (C * cosd(ϕ) - σt*sind(ϕ))^2) - (P * sind(ϕ) + C * cosd(ϕ))

function compute_F(τ, P, C, ϕ, ηvp, σt, λ)
...
F_residual(τ, P, C, ϕ, σt, ηvp, λ, G, dt, η)                       = compute_F(τ, P, C, ϕ, σt, ηvp, λ)
...
function solve(ε, τ, τ0, θ, dt, η, G, λ, P, P0, K, ψ, C, ϕ, σt, ηvp; tol::Float64 = 1.0e-9, itermax = 2, verbose::Bool = false)
